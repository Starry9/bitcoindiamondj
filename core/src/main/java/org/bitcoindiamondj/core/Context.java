...
import org.bitcoindiamondj.utils.ContextPropagatingThreadFactory;
...
public class Context {
    ...
    public Context(NetworkParameters params, int eventHorizon, Coin feePerKb, boolean ensureMinRequiredFee) {
        log.info("Creating bitcoindiamondj {} context.", VersionMessage.BITCOINJ_VERSION);
    ...
    public static Context get() {
        ...
        if (tls == null) {
            if (isStrictMode) {
                log.error("Thread is missing a bitcoindiamondj context.");
                log.error("You should use Context.propagate() or a ContextPropagatingThreadFactory.");
                throw new IllegalStateException("missing context");
            }
            if (lastConstructed == null)
                throw new IllegalStateException("You must construct a Context object before using bitcoindiamondj!");
            slot.set(lastConstructed);
            log.error("Performing thread fixup: you are accessing bitcoindiamondj via a thread that has not had any context set on it.");
     ...
     * want to create core bitcoindiamondj objects. Generally, if a class can accept a Context in its constructor and might
